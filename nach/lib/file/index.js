...
File.prototype.generateBatches = function(done1) {
	...
	async.each(this._batches, function(batch, done2) {
		...
		async.each(batch._entries, function(entry, done3) {
			addendaCount += entry.getRecordCount();
			const traceNumber = ( entry.fields.traceNumber.value) ?  entry.fields.traceNumber.value : self.header.immediateOrigin.value.slice(0,8) + utils.pad(addendaCount, 7, false, '0');
			entry.fields.traceNumber.value = traceNumber;
			if (entry._addendas.length) {
				entry._addendas[0].fields.entryDetailSequenceNumber.value = traceNumber.slice(-7);
			}
			entryHash += Number(entry.fields.receivingDFI.value);

			// Increment the addenda and block count
			// addendaCount += entry._addendas.length;
            rows++;
			if (entry._addendas.length)
				rows++;
